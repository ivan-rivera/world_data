...
# Inputs:
# - s00_init.R
# - data/wdi_extract.Rdata
# - data/dim_country.Rdata
# - data/country_attr_df.Rdata
# - data/bilateral_trade.Rdata
# - data/sankey_data.Rdata
# - data/country_geo.Rdata
# 
# Outputs:
# - forced_net_d3()
# - bt_chord()
...
#source("code/rcharts_pyramids.R") # source from GH
source("code/s00_init.R")      # source init file
load_packages(c(               # load packages (some will be dropped)
  #  "pyramid",                   # * https://rpubs.com/walkerke/pyramids_ggplot2
  ...
  #  "rbokeh",                    # * http://www.htmlwidgets.org/showcase_rbokeh.html
...
plot_map <- function(
  ...
  wdi_data %>% mutate(
    ...
    plot_ly(
      ...
      title = paste("World Map:",indicator_index[indicator_index$label==map_var,"description"]),
      geo   = geo_list
    )
}
...
plot_map(map_decade = 2000, map_var = "birth_rate_p1000", map_type = "flat")
...
# example of use:
p_file <- age_pyramid("New Zealand")
# p_file$save("/Users/Ivan/Documents/Projects/my_shiny_apps/wdi/outputs/pyramid.html", cdn = TRUE)
# can I load CDN outputs into shiny? # https://github.com/ramnathv/rCharts/issues/483

# Alternative Pyramid Plot
pyramid_pl <- function(p_country,p_decade,pyramid_mode="absolute"){
  ...
  plot_ly(p_dat,
    ...
    title = paste0(p_country,plot_title),
    bargap = 0.1,
    barmode = "overlay",
    xaxis = list(
      tickmode = "array",
...
# example of use:
pyramid_pl("Japan",2000,"percent")


# Population Split Barchart -----------------------------------------------
...
pop_split_bar <- function(bar_country){
  ...
  bar_data %>% mutate(
    ...
    rename("Decade" = decade) %>% plot_ly(
      ...
      title = paste0(bar_country,": Rural / Urban Population Split"),
      bargap = 0.1,
      barmode = "stack"
    )
}
...
# plot_ly(
#     x = (stacked_data %>% filter(Type == "Rural"))$decade,
#     y = (stacked_data %>% filter(Type == "Rural"))$Population,
#     #group = Type,
#     fill = "tonexty",
#     mode="lines"
#   ) %>% add_trace(
#     x = (stacked_data %>% filter(Type == "Urban"))$decade,
#     y = (stacked_data %>% filter(Type == "Urban"))$Population,
#     #group = Type,
#     fill = "tonexty",
#     mode="lines"
#   )
...
# values to play with
#unique(wdi_data$var)
#unique(dim_country$region)
#unique(dim_country$subregion)

# panes:
# - Population
#   * total population (pop_total)
#   * urban / rural split (pop_rural_pot)
#   * Age / gender (pop_gender_age)
#   * migration (net_migrat) [no use?]
#X   --> age distribution (pyramid plot) by gender
#X   --> area chart with population over time (with dygraphs?) split by urban vs rural pop
# - Health
#   * health expenditure as % of GDP (health_expend_publ_pogdp, health_expend_priv_pogdp)
#X   --> 3d scatterplot (countries, reg. col) of health expenditure as % of GDP (public vs private) by one of map attributes, dealth_(non)com_p
#X   --> map
# - Economy
#X   --> d3 (scaled / centered) heatmap within region on: 
#          gni_per_capita, external_ds_pgni, cash_surp_def_pc, tax_inc_prof_cap_apor, tax_on_goods_apor, foregin_d_invest_ni, employee_comp_poe
#          deposit_interest_rate, lending_interest_rate, unemploy_total_apol, self_employ_apoe, bank_cap_to_asset_r, govt_dbt_pgdp,
#          income_held_by_h10, gdp_value, military_exp_gdp
#X   --> chord diagram of exports
# - Development
#X   * internet_subscr_p, internet_users_p, mob_subscription_p, gini_index, sci_tech_pub, corrupt_control, political_stability, 
#     int_homicide_per100k, sp_gei, cpia_gender_equal, cpia_transparency, employers_m, employers_f
#   --> single stacked bar charts (or density plots) for each of the above variables
#   --> ALTERNATIVE: density plot for each variable by region -- show country on that density upon selection
# - Energy Use and Sustainability
#   * co2_emissions_mt_pc, electicity_nuclear, electricity_renew, forest_area_p
#X   --> sankey plot of resource consumption 
#   --> barplot for variables (1 of above) can be grouped to subregion / region
# - Languages and Currencies
#X   --> Languages and Currencies network
#  
